...
{
    [SerializeField] private UICanvas playUI;
    [SerializeField] private UICanvas gameOverUI;
    ...
    {
        SetUI<PlayUI>(playUI);
        SetUI<GameOverUI>(gameOverUI);
    }

    public void SetUI<T>(UICanvas ui) where T : UICanvas
    {
        uiCanvas[typeof(T)] = ui;
    }
    ...
    public T GetUI<T>() where T : UICanvas
    {
        if (!IsLoaded<T>())
        {
            string prefabId = "UI/" + GetUIPrefab<T>().name;
            GameObject canvasGO = PhotonNetwork.Instantiate(prefabId, Vector3.zero, Quaternion.identity);
            UICanvas canvas = canvasGO.GetComponent<UICanvas>();
            canvas.transform.SetParent(CanvasParentTF);
